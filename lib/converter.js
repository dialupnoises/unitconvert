	...
	if(!unit || !fromUnit)
	{
		...
		Converter.currencyValue(unitStr.toUpperCase(), function(err, toPrice) {
			Converter.currencyValue(fromStr.toUpperCase(), function(err, fromPrice) {
				...
				var formattedName = (price == 1 ? currency.name : currency.name + 's');
				if(currency.formatted)
					formattedName = (price == 1 ? currency.formatted[0] : currency.formatted[1]);
				return callback(null, price, formattedName);
			});
		});
		return;
	}
