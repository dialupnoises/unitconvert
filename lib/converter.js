	...
	if(!unit || !fromUnit)
	{
		...
		if(!currency || !fromCurrency)
			return callback(createException(
				'Cannot find unit or currency: ' + (currency == null ? unitStr : fromStr) + '.'
			));
...
function findCurrency(str)
{
	...
	{
		if(currencyObj[str])
			return currencyObj[str];
		if(currencyObj[str.toLowerCase()])
			return currencyObj[str.toLowerCase()];
		return null;
	}
	...
	var closestUnit = Object.keys(currencyObj).sort(function(a, b) { return getEditDistance(a, str) - getEditDistance(b, str); });
	if(getEditDistance(closestUnit[0], str) < 4)
		return checkCurrency(closestUnit[0]);
	return null;
}
